...
public class KafkaConsumerSampler<K, V> extends AbstractTestElement
    ...
    private ConsumerRecords<K, V> getConsumerRecords() {
        String deserializerKeyClassName = getDeserializerKey();
        String deserializerValueClassName = getDeserializerValue();
        ConsumerRecords<K, V> records;

        if (deserializerKeyClassName == null || deserializerValueClassName == null) {
            throw new IllegalStateException("Key or Value deserializer is null");
        }

        try{
            Class<?> deserializerKeyClass = Class.forName(deserializerKeyClassName);
            Class<?> deserializerValueClass = Class.forName(deserializerValueClassName);
            ...
            do {
                ...
                records = (ConsumerRecords<K, V>) kafkaConsumer.poll(Duration.ofMillis(Long.parseLong(getPollTimeout())));
            } while (records.isEmpty());

            for(ConsumerRecord<K, V> record : records){
                K key = getTypedValue(record.key(), deserializerKeyClass);
                V value = getTypedValue(record.value(), deserializerValueClass);
    ...
    public String getConsumerSerializerKeyVariableName() {
        return consumerSerializerKeyVariableName;
    }

    public String getConsumerSerializerValueVariableName() {
    ...
    private String getDeserializerKey() {
        return (String) JMeterContextService.getContext().getVariables().getObject(getConsumerSerializerKeyVariableName());
    }

    private String getDeserializerValue() {
        return (String) JMeterContextService.getContext().getVariables().getObject(getConsumerSerializerValueVariableName());
    }
}
