...
public class KafkaProducerSampler<K, V> extends AbstractTestElement
	...
	private ProducerRecord<K, V> getProducerRecord() {
		...
		if (producerSerializerKeyClassName == null || producerSerializerValueClassName == null) {
			throw new IllegalStateException("Key or Value serializer is null");
		}

		try{
			Class<?> serializerKeyClass = Class.forName(producerSerializerKeyClassName);
			Class<?> serializerValueClass = Class.forName(producerSerializerValueClassName);
